...
/*Dolnośląskie -2, Lubelskie -6, Śląskie - 24, Zachodnopomorskie - 32 */
/* Check na numerach */
proc sql;
create table xyz as
...
/* p-value - 0.0012, zatem odrzucam H0, że liczba osób zaliczanych do grona swoich przyjaciół 
    przez niepalących mężczyzn jest w każdym z 4 losowo wybranych województw taka sama */
...
title2 ' Udział kobiet które nie uprawiają aktywnie żadnej formy sportu czy
    ćwiczeń fizycznych';
run; quit;
...
/* Wstepne oszacowanie wskazuje na 7 czynników (wartości własne > 1) */
...
/* usuwam ładunki mniejsze niż 0.5 co do wartości bezwzględnej jako nieistotne */

data proba_czynniki7;
set proba_czynniki7;

    /* tworzę tablicę ze zmiennymi */
	array zmienne(7) factor1-factor7;

	/* dzięki temu możemy się odwoływać do poszczególnych 
	   zmiennych w pętli jako do kolejnych elementów tablicy,
	   co ułatwia wykonanie tej samej operacji na dużej 
	   grupie zmiennych */
...
/* zapisuję 7 czynników do zbioru */
...
/* standaryzuję uzyskane czynniki do przedziału [0,1],
	bo łatwiej je wtedy porównywać między sobą */
...
/* nadaję nazwy poszczególnym czynnikom */

data proba_factors;
set proba_factors;
	label factor1='F1. Przywiązanie do wartości materialnych';
	label factor2='F2. Niedocenianie niektórych ludzi';
	label factor3='F3. Pozytywne nastawienie do życia';
	label factor4='F4. Konsumpcjonizm';
	label factor5='F5. Legalizacja związków partnerskich';
	label factor6='F6. Przywrócenie kary śmierci';
...
/* Niestety ze względu na ujemne statystyki CCC nie widac do konca poprawnego rozwiazania, dodatkowo cieżko doszukac sie maksymalnej wartosci dla obu statystyk (wylaczajac 0 skupien)
/* Chcielibyśmy aby CCC oraz pseudo-F byly jak najwyzsze ( i dodatnie), natomiast w tym przypadku liczba podzialu bylaby rowna 0 */
/* Dlatego więc, wezmę 3 skupienia, ponieważ wydaje mi się, iż w tym miejscu występuje pewne załamanie się oby dwu wartości */
/* Uznaje, ze NA TEN MOMENT jest to optymalna liczba grup */


/* Niemniej pobawmy się jeszcze chwile i wykluczmy mało różnicujące segmenty, a następnie ponownie sprawdzmy pseudo-F i CCC */
...
/* Usune 2 factory, ze względu na najwyższe odchylenia - factor6 i factor7 */
...
/* Udało się uzyskać trochę więcej załamań, natomiast CCC nadal ma ujemne wartości, co wskazuje na zróżnicowanie pomiędzy segmentami */
/* Nie mniej jednak, dla 4 segmentów zauważamy, iż zdecydowanie załamanie. */
/* Uznajmy więc, iż jest to poprawny wynik */


/* Do tej pory,wszystko robiliśmy losowo - sprawdzmy więc, co sie stanie, gdy wspomożemy się hierarchiczną metodą */
...
/* Wydaje się, iż CCC lekko wzrosło z poziomu około -26 do -45 oraz Pseudo-F wzrosło z około 1500 (?) do 1827 */
...
/* check dla zarobkow */
/* w proc sql nie ma over partition by, więc */
proc sql;
...
/* przeciętne pragnienie życia - HP40 */
...
/* proporcja osob z wykształceniem conajmniej średnim */
...
set proba_4a_factors;
	...
	label factor2='F2. Przeciętne pragnienie do życia';
	label factor3='F3. Mediana dochodu miesięcznego netto';
	label factor4='F4. Średni indeks BMI';
	label factor5='F5. Proporcja osób, które w minionym tygodniu oglądały telewizję mniej niż przez 2 godziny dziennie';
	label factor6='F6. Proporcja osób z wyksztaceniem co najmniej średnim';
	label factor7='F7. odstęp międzykwartylowy dla liczby osób zaliczanych do przyjaciół';
run;
...
/* Według mnie powinno być 2-6 segmentow */
...
%sgplotsegh(tree=proba4_treeF, /* nazwa zbioru z dendrogramem */
            nseg=9, /* liczba grup, na które chcemy podzielić
			           obserwacje i porównać na wykresie */
           /* lista zmiennych, których średnie wartości
			  w poszczególnych segmentach chcemy porównać.
...
/* ----------------------------------------------------------------- */
/* Zadanie 5 */
/* a. Zweryfikuj hipoteze, ze rozkład pragnienia życia (w tych dniach)
   dla osób, które nie były w minionym miesiącu:
   - w kinie, teatrze lub na koncercie, ani
   - na spotkaniu towarzyskim   
	nie zalezy od klasy wielkości miejscowości zamieszkania */
/* 2.5 punktu */
...
/* W obu przypadkach brak normalności */
...
/* ----------------------------------------------------------------- */
/* b. Zweryfikuj hipoteze, ze częstość udziału w nabożeństwach 
      osób mieszkających w największych miastach nie zalezy od plci. */
/* 2.5 punktu  */
...
/* H0 : częstość udziału w nabożeństwach osób w największych miastach jest taka sama niezależnie od płci (nie zależy od płci)
p-value = 0.001 < 0.05 -> odrzucam h0 */
	

/* ----------------------------------------------------------------- */
/* c. Zweryfikuj hipoteze, ze srednie zadowolenie z pracy 
   i ze swego wyksztalcenia dla osób, które przynajmniej 1 godzinę
   tygodniowo poświęcają na czytanie prasy, są sobie rowne */
/* 2.5 punktu */
...
/* ----------------------------------------------------------------- */
/* d. Osobno dla kobiet i dla mężczyzn zweryfikuj hipoteze, ze 
   taki sam procent osób uważa, że ogólnie rzecz biorąc większości ludzi 
   można ufać oraz, że  demokracja ma przewagę nad wszelkimi innymi formami 
   rządów */
/* 2.5 punktu */
...
/* p-value <.0001 -> odrzucam h0, ze taki sam procent mezczyzn uważa, że ogólnie rzecz biorąc większości ludzi 
   można ufać oraz, że  demokracja ma przewagę nad wszelkimi innymi formami rządów */
...
/* p-value <.0001 -> odrzucam h0, ze taki sam procent kobiet uważa, że ogólnie rzecz biorąc większości ludzi 
   można ufać oraz, że  demokracja ma przewagę nad wszelkimi innymi formami rządów */


/* ----------------------------------------------------------------- */
/* e. Uzywajac stosownych testow statystycznych odpowiedz na pytanie, 
    czy zadowolenie ze sposobu spędzania wolnego czasu zależy od
	statusu społeczno-zawodowego respondenta. */
/* 2.5 punktu */
...
/* H0: Zadowolenie ze sposobu spędzania wolnego czasu nie zależy od statusu społeczno-zawodowego respondenta
p-value <.0001 -> odrzucam H0 */

/* ----------------------------------------------------------------- */
/* f. Policz i podaj interpretacje miary/miar wspolzaleznosci oceny całego swojego 
    dotychczasowego życia i miesiecznego dochodu (na reke). */
/* 3 punkty */
...
/* obie zmienne nie są z rozkładu normalnego tak więc korelacja Spearmana (albo Kendall Tau) */
...
/* ----------------------------------------------------------------- */
/* g. Narysuj i zinterpretuj wykres podstawowych parametrow rozkładu 
	liczby godzin korzystania z Internetu w ostatnim tygodniu
	w zależności od grupy wiekowej.
	Ogranicz się do osob, które nigdy nie płaciły za treści dostępne 
    w Internecie */
/* 4 punkty */
